...
class KeyVault(peewee.Model):
    ...
    status = CharField(default='pending', max_length=50, null=True)
    enabled = BooleanField(default=True, null=True)
...
def migrate_forward(op, old_orm, new_orm):
    op.add_column(new_orm.keyvault.status)
    op.add_column(new_orm.keyvault.enabled)
    op.run_data_migration()


def migrate_backward(op, old_orm, new_orm):
    op.run_data_migration()
    op.drop_column(old_orm.keyvault.status)
    op.drop_column(old_orm.keyvault.enabled)
